...
$leaderboard = [];
$sql = "SELECT u.fullname, u.role, MAX(qa.percentage) as best_percentage,
...
<head>
    ...
    <style>
        .leaderboard-gradient { background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); }
        .podium-1 { background: linear-gradient(135deg, #FFD700, #FFA500); transform: scale(1.1) translateY(-10px); z-index: 3; }
        .podium-2 { background: linear-gradient(135deg, #C0C0C0, #A9A9A9); transform: scale(1.05); z-index: 2; }
        .podium-3 { background: linear-gradient(135deg, #CD7F32, #A0522D); z-index: 1; }
        .podium-item { transition: all 0.3s ease; color: white; border-radius: 1rem; box-shadow: 0 10px 25px -5px rgba(0, 0, 0, 0.25); }
                    ...
                    <?php if (isLoggedIn()): ?>
                        <?php if (isStudent()): ?>
                            <a href="student/quiz.php" class="flex items-center space-x-2 text-gray-700 hover:text-indigo-600 transition duration-200">
                                <i class="fas fa-play-circle"></i><span>Take Quiz</span>
                            </a>
                ...
                <div class="bg-white rounded-3xl shadow-xl p-16 text-center">
                    ...
                    <?php if (isLoggedIn() && isStudent()): ?>
                        <a href="student/quiz.php" class="inline-flex items-center bg-gradient-to-r from-indigo-600 to-purple-600 text-white px-8 py-4 rounded-2xl text-lg font-semibold hover:from-indigo-700 hover:to-purple-700 transition duration-200 transform hover:scale-105">
                            <i class="fas fa-play mr-3"></i>Take Your First Quiz
                        </a>
                    <?php else: ?>
                        <a href="auth/register.php" class="inline-flex items-center bg-gradient-to-r from-indigo-600 to-purple-600 text-white px-8 py-4 rounded-2xl text-lg font-semibold hover:from-indigo-700 hover:to-purple-700 transition duration-200 transform hover:scale-105">
                            <i class="fas fa-user-plus mr-3"></i>Join Now
                        </a>
                    <?php endif; ?>
                </div>
            <?php else: ?>
                ...
                <div class="flex justify-center items-end mb-16 space-x-4">
                    ...
                    ?>
                        <div class="podium-item podium-<?= $position ?> w-48 p-6 text-center">
                            <div class="mb-4">
                                <?php if ($position === 1): ?><i class="fas fa-crown text-4xl mb-2"></i>
                                <?php elseif ($position === 2): ?><i class="fas fa-medal text-4xl mb-2"></i>
                                <?php else: ?><i class="fas fa-award text-4xl mb-2"></i><?php endif; ?>
                            </div>
                            <div class="text-5xl font-bold mb-2">#<?= $position ?></div>
                            <div class="font-bold text-xl mb-3"><?= htmlspecialchars($user['fullname']) ?></div>
                ...
                <div class="bg-white rounded-3xl shadow-xl overflow-hidden">
                    <div class="bg-gradient-to-r from-indigo-600 to-purple-600 px-8 py-6">
                        <h2 class="text-2xl font-bold text-white flex items-center">
                            <i class="fas fa-list-ol mr-3"></i>Complete Rankings
                        </h2>
                    </div>
                                ...
                                <div class="flex items-center justify-between">
                                    <div class="flex items-center space-x-6">
                                        <div class="w-16 h-16 <?= $rank <= 3 ? 'bg-gradient-to-r from-yellow-400 to-yellow-600' : 'bg-gradient-to-r from-gray-400 to-gray-600' ?> rounded-full flex items-center justify-center text-white font-bold text-xl">
                                            ...
                                            <?php elseif ($rank === 2): ?><i class="fas fa-medal"></i>
                                            <?php elseif ($rank === 3): ?><i class="fas fa-award"></i>
                                            <?php else: ?><?= $rank ?><?php endif; ?>
                                        </div>
                                        <div>
                                            <h3 class="text-xl font-bold text-gray-800"><?= htmlspecialchars($user['fullname']) ?></h3>
                                            <p class="text-gray-600"><?= $user['total_attempts'] ?> attempts • Avg: <?= $avg_percentage ?>%</p>
                                        </div>
                                    </div>
    ...
    <footer class="bg-gray-900 text-white py-12">
        <div class="container mx-auto px-4 text-center">
            <div class="flex items-center justify-center space-x-3 mb-4">
                ...
                <h3 class="text-xl font-bold bg-gradient-to-r from-indigo-400 to-purple-400 bg-clip-text text-transparent">QuizMaster Pro</h3>
            </div>
            <p class="text-gray-400">&copy; 2025 QuizMaster Pro. All rights reserved.</p>
        </div>
    </footer>
</body>
</html>
